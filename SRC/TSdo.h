...
private:
	...
	//sdoファイルを展開する
	bool expandSDO(const std::wstring sdo_file,std::unique_ptr<TMemIniFile>& pIni,std::wstring& TmpIniPath);
	//基礎情報のセット
	bool setBasicInfo(std::unique_ptr<TMemIniFile>& pIni,const std::wstring& sdo_file,const std::wstring& fileID,typDocument& DocInfo);
	//部品情報の読み出し
	...
	//sdoファイルを作成する
	bool makeSDO(const String& TmpDirStr,std::unique_ptr<TStringList>& pFiles,const std::wstring& FilePath);
...
public:
	//.sdoファイルの読み込み
	bool readSDO(const std::wstring& fileID,typDocument& DocInfo);
	//テンプレート.sdoファイルの読み込み
	bool readTemplateSDO(const std::wstring& filePath,typDocument& DocInfo);
	//空読み込み
	bool readVoid(typDocument& DocInfo);
	//.sdoファイルの保存
	bool writeSDO(typDocument& DocInfo,const std::wstring& filePath);
};
